...
def chroot_execute( cmd, stdin=None ):
  _execute( '/sbin/chroot %s %s' % ( chroot_path, cmd ), debug_stdout, debug_stdout, stdin, chroot_env )

def chroot_execute_lines( cmd, stdin=None ):
  stdout = _execute( '/sbin/chroot %s %s' % ( chroot_path, cmd ), subprocess.PIPE, debug_stdout, stdin, chroot_env )
  return stdout.splitlines()
